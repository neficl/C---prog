...
void CompraFeita(double carrinho[TAM]){ //Com a compra finalizada iremos exibir a quantidade dos produtos, assim como seus preços unitarios e o valor total gasto.
    ...
    for(i = 0; i < TOTAL_PRODUTOS; i++){
        if(carrinho[i] != 0){ //Lembrando que utiliamos a passagem de parametro para o nosso array que foi executada no comando principal main.
            quantidade = carrinho[i] / compra[i].valorProduto; //Calculo responvel por definir a quantidade de cada produto lido.
            if(quantidade != 0){
            printf("%dx %s: R$ %.2lf\n", quantidade, compra[i].produto, carrinho[i]); //Impressao dos resultados.
            }
        }
    }
    puts("-----");
...
int main(void){
    ...
    CompraFeita(carrinho); //Chamada da função que retorna os gastos e informações das compras, passamos por parametro as informações de produto e preço.
