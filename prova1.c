    ...
    if(M > N){ //trocar os valores
        ...
        for(i = M; i < N; i++){
             if(i % 2 == 0 && P % 2 == 0){ // Descobriremos se o valor da sentença é par, se sim vamos apresentar os valores que se encaixam dentro da variavel N(Maior) e M(menor).
                printf("%d ", i);
             }else if(i % 2 ==1 && P % 2 == 1){ // Descobriremos o valor da sentença se é impar, se sim, apresentaremos os valores.
                printf("%d ", i);
             }else if(i % 2 == 0 && P % 2 == 0 && N){
                printf("%d \n", i); // Adicionamos essa saída para qie uma nova linha posso ser adicionado um novo caractere.
             }else if(i % 2 ==1 && P % 2 == 1 && N){
                printf("%d \n", i);
             }else if(M == N){ // Ao paço pelas duas condições ifs e não forem verdadeiras, nossa condição else será executada!
